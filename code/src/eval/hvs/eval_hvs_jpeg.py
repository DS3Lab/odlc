...
class EvalMSSSIMJpeg(EvalMSSSIMBase):
    COMPRESSION_LEVELS = [5, 10, 15, 20, 30, 50, 70, 95]
    NUM_PREPROCESSING_THREADS = 16

    def __init__(self, dataset_name, records_file, batch_size):
        # rnn specs
        logger, logfile = self._get_logger(dataset_name, 'jpeg')
        super(EvalMSSSIMJpeg, self).__init__(dataset_name, logger, records_file)
    ...
    def run(self):
        ...
        with tf.Graph().as_default() as graph:
            ...
            for j, compression_level in enumerate(self.COMPRESSION_LEVELS):
                # compress batch
                with tf.name_scope('compression_jpeg_{}'.format(compression_level)):
        ...
        with tf.Session(config=get_sess_config(allow_growth=True), graph=graph) as sess:
            ...
            self._save_results(mean_bpp_values_per_compression, mean_dist_values_per_compression, 'jpeg',
                               self.COMPRESSION_LEVELS)
