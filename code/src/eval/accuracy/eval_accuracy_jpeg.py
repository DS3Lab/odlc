...
os.environ["CUDA_VISIBLE_DEVICES"] = "7"  # noqa
...
class EvalAccuracyJpeg(EvalAccuracyBase):
    ...
    def __init__(self, dataset_name, records_file, checkpoint_root=None):
        logger = self._get_logger(dataset_name, 'jpeg')
        super(EvalAccuracyJpeg, self).__init__(dataset_name, records_file, logger, checkpoint_root)

        self.logger.info('compression_method=jpeg')
    ...
    def _save_results(self, bpp, accs, cnn_model_name):
        ...
        csv_file = os.path.join(self._log_dir, 'jpeg_{}_accuracy_{}.csv'.format(cnn_model_name, self._log_number))
        ...
        with tf.Graph().as_default() as graph:
            ...
            for j, compression_level in enumerate(self.COMPRESSION_LEVELS):
                with tf.device(_CPU_DEVICE):
                    image_batch_compressed, bpp = TFJpeg.encode_decode_image_batch(
                        image_batch=images,
                        quality=compression_level,
                        image_shape=image_shape_compression)

                with tf.name_scope('inference_jpeg{}'.format(compression_level)):
