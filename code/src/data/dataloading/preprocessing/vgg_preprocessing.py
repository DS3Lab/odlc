...
#
# ============================================================================================================
""" Provides utilities to preprocess images for networks that use VGG preprocessing. """
...
class VGGPreprocessing:

    @classmethod
    def preprocess_image(cls, image, output_height, output_width, mean_center_image=True, is_training=False,
        ...
        Args:
          ...
          mean_center_image: If True, per channel Imagenet means are subtracted from the image.
        ...
        if is_training:
            ...
            return cls.preprocess_for_eval(image, output_height, output_width, resize_side_min, mean_center_image)

    @classmethod
    def preprocess_for_eval(cls, image, output_height, output_width, resize_side, mean_center_image=True):
        ...
        if mean_center_image:
            return cls.mean_image_subtraction(image, [_R_MEAN, _G_MEAN, _B_MEAN])
        else:
            return image

    @classmethod
    def preprocess_for_train(cls, image, output_height, output_width, resize_side_min=_RESIZE_SIDE_MIN,
                             resize_side_max=_RESIZE_SIDE_MAX, mean_center_image=True):
    ...
    @staticmethod
    def mean_image_subtraction(image, means=None):
    ...
    # @staticmethod
    # def standardize_image_batch(image_batch, means):
    #     """Subtracts the given means from each image channel.
    #
    #
    #     For example:
    #       means = [123.68, 116.779, 103.939]
    #       image_batch = _mean_image_batch_subtraction(image_batch, means)
    #     Note that the rank of `image` must be known.
    #
    #     Args:
    #       image_batch: a tensor of size [batch_size, height, width, C].
    #       means: a C-vector of values to subtract from each channel.
    #
    #     Returns:
    #       the centered image_batch.
    #
    #     Raises:
    #       ValueError: If the rank of `image_batch` is unknown, if `image_batch` has a rank other than four or if the
    #       number of channels in `image_batch` doesn't match the number of values in `means`.
    #     """
    #
    #     if image_batch.dtype != tf.float32:
    #         image_batch = tf.to_float(image_batch)
    #
    #     if image_batch.get_shape().ndims != 4:
    #         raise ValueError('Input must be of size [batch_size, height, width, C>0]')
    #
    #     num_channels = image_batch.get_shape().as_list()[-1]
    #
    #     if len(means) != num_channels:
    #         raise ValueError('len(means) must match the number of channels')
    #
    #     channels = tf.split(axis=3, num_or_size_splits=num_channels, value=image_batch)
    #
    #     for i in range(num_channels):
    #         channels[i] -= means[i]
    #
